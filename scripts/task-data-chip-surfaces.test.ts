...
function run(): void {
	...
	const lightboxSource = readFileSync(resolve(process.cwd(), 'src/ui/lightbox.ts'), 'utf8');
	assert.match(lightboxSource, /lightbox\.setAttribute\('aria-modal', 'true'\)/u);
	assert.match(mediaPreviewSource, /activeTaskMediaPreviews\.get\(ownerDocument\)\?\.\(\)[\s\S]*openLightbox\(anchor,/u);
	assert.match(lightboxSource, /operon-task-media-lightbox-title'[\s\S]*text: options\.title/u);
	assert.match(mediaPreviewSource, /title: source\.label/u);
	assert.match(mediaPreviewSource, /operon-task-media-lightbox-content is-\$\{source\.kind\}/u);
	assert.match(lightboxSource, /event\.target === lightbox\) close\(\)/u);
	assert.match(lightboxSource, /event\.key === 'Escape'[\s\S]*close\(\)/u);
	assert.match(lightboxSource, /removeEventListener\('focusin', keepFocusInside, true\)[\s\S]*contentCleanup = null[\s\S]*cleanup\?\.\(\)/u);
	assert.match(lightboxSource, /if \(isClosed\) return;[\s\S]*isClosed = true/u);
	assert.match(lightboxSource, /addEventListener\('focusin', keepFocusInside, true\)[\s\S]*closeButton\.focus/u);
