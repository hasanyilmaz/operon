...
Operon has a lot of settings, spread across tabs for Core, Tasks, Views, Interface, and Mobile. You do not have to remember where each one lives. Operon registers every setting with Obsidian's built-in settings search, so you can search for a setting by name or by what it does and jump straight to it.
...
- **Stable.** Searching always works, even after the tab layout changes, because it keys off the settings themselves.
...
**Where is the search box?** At the top of Obsidian's Settings window. It searches all plugins, Operon included.
