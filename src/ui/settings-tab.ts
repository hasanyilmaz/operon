...
export interface DeveloperApiSettingsGrantV1 extends DeveloperApiGrantRecordV1 {
...
export class OperonSettingsTab extends PluginSettingTab {
	...
	private renderDeveloperApiIntegrations(containerEl: HTMLElement): void {
		...
		for (const grant of grants) {
			...
			const reactivationCapabilities = grant.grantedCapabilities.filter(capability => approvalSet.has(capability));
			const pendingApprovalCapabilities = grant.pendingCapabilities.filter(capability => approvalSet.has(capability));
			const selected = new Set(reactivationCapabilities);
			const approvalBlocked = grant.suspensionReason === 'consumer-version-invalid';
			let approveButton: ButtonComponent | null = null;
			const updateApprovalButton = (): void => {
				approveButton?.setDisabled(approvalBlocked || !grant.approvalBinding || selected.size === 0);
			};
			...
			if (showsApprovalControls) {
				...
				const renderCapabilities = (
					...
					capabilityList.createEl('p', { text: label, cls: 'operon-settings-muted-block' });
