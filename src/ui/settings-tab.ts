...
export class OperonSettingsTab extends PluginSettingTab {
	...
	constructor(
		...
		_developerApiIntegration?: DeveloperApiSettingsIntegration,
   ...
   items: [
	...
	private normalizeSettingsSearchDropdownValue(key: OperonSettingSearchKey, value: unknown): unknown {
		...
		if (key === 'dateDisplayFormat') {
			return normalizeDateDisplayFormat(text);
		}
 ...
 private configureTaskCardSetting(setting: Setting, key: keyof TaskCardSettings): void {
  ...
  if (isTaskCardSetting(key)) return this.taskCardDropdownOptions(key);
		...
		if (key === 'dateDisplayFormat') {
			return getDateDisplayFormatDropdownOptions();
		}
	...
	private applySettingsSearchAfterSaveEffects(key: OperonSettingSearchKey): void {
		...
		if (key === 'timeFormat' || key === 'dateDisplayFormat' || key === 'reminderSoundFilePath') {
			this.updateNativeSettingsDefinitions();
		}
