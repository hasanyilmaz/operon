...
- Added **Changed routing** in open Canvases: when a task no longer matches a single-value Operon group, its existing card moves into a matching subgroup inside a normal Changed group. Cleared values go directly into Changed and remain tracked for later updates; list groups do not trigger automatic movement. Undo restores the Canvas layout without reverting the task change or immediately repeating the move.
