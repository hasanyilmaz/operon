...
- Added **Operon Groups** to the Canvas creation menu, with searchable properties and existing pickers for single-property group titles. Text selections save immediately; list selections keep the title open for more values. Edit existing group labels with the same controls and undo or redo each saved change. Task dropping and automatic movement are not enabled in this stage.
