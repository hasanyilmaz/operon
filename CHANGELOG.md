...
- Added **Operon Groups** to the Canvas creation menu, with searchable properties and existing pickers for single-property group titles. Text selections save immediately; list selections keep the title open for more values. Edit existing group labels with the same controls and undo or redo each saved change. Drag one existing task card into a valid group to preview and apply its rule, adding missing list values together. Card movement and task changes share one Canvas Undo/Redo step; blocked entries return to their previous position. Groups using the same single-value property cannot overlap: conflicting moves and resizes return the group and any carried cards to their previous positions with a notice. Creation, title edits, keyboard moves, copy-dragging, and automatic growth follow the same rule; list groups remain unrestricted and existing overlaps can be separated.
- Added **automatic Operon Group routing** in open and closed Canvases while Operon is running: when a task no longer matches a single-value Operon group, its existing card first moves into an available matching group in the normal Canvas area. Placement uses existing space or safe expansion down, right, up, then left without moving surrounding cards. If no normal target has room, the card moves into a matching subgroup inside a normal Group Mismatches group. Cleared values go directly into Group Mismatches and remain tracked for later updates; list groups do not trigger automatic movement. Open Canvas Undo restores the layout without reverting the task change or immediately repeating the move. Closed Canvas updates preserve existing cards and connections, coordinate with opening, saving, and closing views, and use available file history for recovery. Other open views cannot overwrite a newer automatic layout with stale content, and synchronization follows folder renames. Cards in Group Mismatches are reconsidered when suitable normal groups become available; empty routing groups are removed safely and restored by layout Undo when applicable.
- Added **Property Value Pool settings** with nine configurable shortcut slots. Each slot offers the same property dropdown, including custom fields, All values, Favorites, Dates, and No value, with visibility and ordering controls. All values and Favorites lead the initial order, duplicate assignments are prevented, and shared favorites remain manageable even when their values become unavailable. Media favorites refresh when files are created, deleted, or renamed.
- Added a **Property Value Pool** panel to Canvas with property shortcuts, scoped value search, and shared favorites that stay synchronized across open panels and Settings. Move and pin the panel alongside Task Pool to keep frequently used values close at hand. Search and pin controls keep a single changing icon, the search field uses one focus frame, and favorite stars work without hover tooltips or filled button backgrounds. Value text and framed icons match Task Pool sizing. Favorites show just the property icon and value, and pin colors match Task Pool. The panel opens in the first visible shortcut; Clear or an extra Backspace in an empty search returns to that same group; arrow keys navigate results and Enter toggles the selected value’s favorite.
- Added **combined favorite and property search** to Property Value Pool, with matching favorites followed by property suggestions. Left and right arrow keys switch visible groups while the search box is empty. Property suggestions use the same framed-icon cards as values, without stars or redundant tooltips. Up from the first result focuses the shortcut row; left/right immediately switches groups and refreshes results without clearing the search, Enter or Space returns to the search, and Down restores the caret at the end of the preserved text. Clicking top shortcuts also keeps the query, and top shortcuts keep neutral icons and surfaces, with an accent border for the selected group and a neutral hover border matching Task Pool. Shortcut spacing spreads across the search width without shrinking icons or tablet targets. Shortcut navigation handles intercepted keyboard events without trapping focus.
- Added a configurable **All values** shortcut in Property Value Pool to search supported property values together, with property suggestions appearing first after two characters for quick keyboard scope changes. Cached sources are reused across searches and favorite changes. The result counter labels entries as values rather than tasks.
...
- Added **dynamic date values** to Property Value Pool for Due, Scheduled, Started, Completed, Cancelled, and custom date fields. An optional **Dates** shortcut searches these fields together while keeping their individual groups and field-specific favorites. Search 23 relative choices in English, Turkish, or the interface language; favorites keep rules such as Tomorrow current, previews show the actual date, and Undo/Redo restores the applied dates and related changes together. Scheduled changes follow the existing daily/weekly parent rules and create a missing periodic note when needed; Undo restores the prior date and parent while retaining the shared note.
- Added **ReminderRules** to Property Value Pool with the existing On time, 10m, 30m, 1h, and 1d presets plus other rules already used by tasks. Search, favorite, and append rules with an exact reminder-time preview and Canvas Undo/Redo; missing reference times, past reminders, duplicates, and stale previews are blocked without changing absolute reminders.
- Added mouse and pen dragging from **Property Value Pool** onto a Canvas task, with a change preview and native Canvas Undo/Redo. The drag card keeps the source row’s size, icon, and value without its favorite star. List values are added without duplicates; stale or workflow-dependent changes are blocked before writing. Completing or cancelling a task with a running timer and editing a recurring task’s schedule require Task Editor; these checks also protect Undo/Redo and changes made while a drop is pending.
...
- Reduced repeated rule resolution, Property Value Pool source scans, and Canvas placement work so task updates refresh affected cards and values more efficiently.
- Improved Canvas panning by retaining relation icons and measuring only task cards whose content or size changed, while preserving automatic height adjustment.

### Fixed

- Fixed valid Canvas parent–child links being rejected when automatic modification timestamps differed from the stored file value.

### Validation

- Local Plugin and Runtime checks, production build, and release guard passed; the local regression suite passed **3,037/3,037** tests, including 13 group-overlap checks for geometry rollback, native history, creation and title validation, copying, keyboard movement, and automatic placement, plus 24 integrated Operon Group checks for editor-to-drop Undo/Redo, guarded inline/YAML writes, open/closed routing parity, persistent Undo suppression, multiple-view save protection, folder transitions, listener cleanup, and incremental updates across 40 synthetic Canvases. Coverage also includes 62 closed-Canvas checks covering incremental references, atomic writes, opening/saving and rename races, lossless JSON, uncertain outcomes, shutdown, and zero task writes, and 57 Changed-routing checks covering scalar and empty values, layout preservation, persistent Undo suppression, multiple views, interaction coordination, save recovery, and zero task writes, plus native mouse/pen/tablet group drops, multi-value writes, combined movement/task Undo/Redo, stale-preview and source-conflict protection, uncertain-write recovery, and Operon Group creation, picker title editing, raw list/media values, cancellation, native Undo/Redo, viewport and listener cleanup, syntax, field validation, escaped lists, target geometry, tracking compatibility and zero-write checks, active-timer and recurring-schedule write guards, Settings media refresh and listener cleanup, first-shortcut opening and reset, grouped date search, custom-name compatibility, nine-slot shortcuts, duplicate prevention, empty-slot metadata, settings backup/restore, Pool value sources, link/media references, preference compatibility, tablet interactions, and inline/YAML schedule, gallery, and dynamic-date Undo/Redo, including clock changes, periodic parent creation, cancellation, parent Undo/Redo, and reminder-rule previews, duplicate checks, keyboard and tablet drops. Reminder scheduler tests passed **146/146**. The related settings preservation, version backup, restore, and reset checks passed **201/201** tests.
...
- Fixed desktop and mobile **Mark done**, **Cancel**, checkbox, status-cycle, Task Editor save, and Kanban move actions depending on Runtime readiness. These everyday UI changes now use the same Plugin-native save path on both platforms.
- Fixed Task Editor terminal saves with an active timer writing the task twice. Timer totals and sessions are now included in the same single task write.
- Fixed Task Editor deletion failing on desktop or Android when related task sources were open or Runtime admission was unavailable. Open buffers are synchronized first, the target is removed once, and uncertain deletions are never retried automatically.
- Fixed deleted tasks leaving broken Operon-owned relationships. Direct children are detached, and the deleted task is removed from **Blocking** and **Blocked By** while user-managed **Related** values remain unchanged.
