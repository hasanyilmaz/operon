...
- Extended **Property Value Pool** to find existing task types, used icons, named palette colors, and used hex colors. Color swatches and actual icons appear in results, favorites, Settings, and drag cards; optional shortcuts start hidden, and existing favorites are preserved.
...
- Local Plugin and Runtime checks, production build, and release guard passed; the local regression suite passed **2,285/2,285** tests, including Pool type/icon/color sources, preference compatibility, tablet interactions, and inline/YAML Undo/Redo. The related settings preservation, version backup, restore, and reset checks passed **201/201** tests.
