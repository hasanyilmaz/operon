...
- Added **combined favorite and property search** to Property Value Pool, with matching favorites followed by property suggestions. Left and right arrow keys switch visible groups while the search box is empty. Property suggestions use the same framed-icon cards as values, without stars or redundant tooltips. Up from the first result focuses the shortcut row; left/right moves between shortcuts, Enter or Space opens a group without clearing the search, and Down returns to the preserved search text. Clicking top shortcuts also keeps the query, and top shortcuts keep neutral icons and surfaces, with an accent border for the selected group and a neutral hover border matching Task Pool. Shortcut navigation handles intercepted keyboard events without trapping focus.
- Added an **All values** shortcut beside Favorites in Property Value Pool to search supported property values together, with property suggestions appearing first after two characters for quick keyboard scope changes. Cached sources are reused across searches and favorite changes.
...
- Local Plugin and Runtime checks, production build, and release guard passed; the local regression suite passed **2,317/2,317** tests, including Pool value sources, link/media references, preference compatibility, tablet interactions, and inline/YAML schedule and gallery Undo/Redo. The related settings preservation, version backup, restore, and reset checks passed **201/201** tests.
