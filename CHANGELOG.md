...
- Added **Changed routing** in open and closed Canvases while Operon is running: when a task no longer matches a single-value Operon group, its existing card moves into a matching subgroup inside a normal Changed group. Cleared values go directly into Changed and remain tracked for later updates; list groups do not trigger automatic movement. Open Canvas Undo restores the layout without reverting the task change or immediately repeating the move. Closed Canvas updates preserve existing cards and connections, coordinate with opening, saving, and closing views, and use available file history for recovery. Other open views cannot overwrite a newer automatic layout with stale content, and synchronization follows folder renames.
